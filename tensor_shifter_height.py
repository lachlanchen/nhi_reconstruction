...
class ShiftCalculator:
    def __init__(self, height=8, steps=260, lines_per_mm=600, distance=84, reverse=False, adjustment_factor=.8, adjust_at='distance'):
        self.height = height  # Height of the sensor in mm
        self.steps = steps  # Number of discrete steps along the height
...
if __name__ == "__main__":
    # Define parameters
    height = 8  # mm
    steps = 260  # Number of steps along the height dimension
    lines_per_mm = 600
    distance = 84  # mm
    num_wavelength_steps = 13  # Arbitrary choice

    # Create a tensor of shape [260, 346, 13] with random values
    initial_tensor = torch.randn(steps, 346, num_wavelength_steps)

    # Instantiate the ShiftCalculator and compute the shift vector
    shift_calculator = ShiftCalculator(height=height, steps=steps, lines_per_mm=lines_per_mm, distance=distance)
