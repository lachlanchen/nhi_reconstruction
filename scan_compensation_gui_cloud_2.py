...
Enhanced with manual parameter input and optional training
"""
...
class ScanCompensationGUI:
    ...
    def create_widgets(self):
        ...
        self.ax_var = tk.DoubleVar(value=0.0)
        ...
        self.ay_var = tk.DoubleVar(value=0.0)
        ...
        self.enable_training_var = tk.BooleanVar(value=True)
        ...
        ttk.Label(param_frame2, text="Min λ (nm):").grid(row=1, column=0, sticky="w")
        self.min_wave_var = tk.DoubleVar(value=380)
        ttk.Entry(param_frame2, textvariable=self.min_wave_var, width=10).grid(row=1, column=1, padx=2)
        
        # Wavelength and Actions - Row 2, Column 5
        action_frame = ttk.LabelFrame(control_frame, text="Actions", padding=5)
        action_frame.grid(row=1, column=4, sticky="ew", padx=2, pady=2)
        
        ttk.Label(action_frame, text="Max λ (nm):").grid(row=0, column=0, sticky="w")
        self.max_wave_var = tk.DoubleVar(value=780)
        ttk.Entry(action_frame, textvariable=self.max_wave_var, width=10).grid(row=0, column=1, padx=2)
        
        # Action buttons
        button_subframe = ttk.Frame(action_frame)
        button_subframe.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(5, 0))
        
        self.run_button = ttk.Button(button_subframe, text="Run/Set Parameters", command=self.run_optimization)
        self.run_button.pack(side=tk.LEFT, padx=(0, 2))
        
        ttk.Button(button_subframe, text="Generate 3D", command=self.generate_3d_vis).pack(side=tk.LEFT)
    ...
    def create_3d_plot(self):
        ...
        try:
            ...
            if len(ts_sampled) > 0:
                ...
                time_stretch = 10.0  # Fixed stretch factor
                ...
                ax.set_title(f'3D Compensated Events (Spectral) - {mode}\n'
