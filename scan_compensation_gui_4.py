...
Improved with proper time stretching, index_put accumulation, frame saving, and clean UI
"""
...
class ScanCompensation(nn.Module):
    ...
    def generate_frames_only(self, x_coords, y_coords, timestamps, polarities, H, W, bin_width):
        """Generate frames without gradients - for saving frames after optimization"""
        with torch.no_grad():
            x_warped, y_warped, t_warped = self.warp(x_coords, y_coords, timestamps)
...
class ScanCompensationGUI:
    def __init__(self, root):
        ...
        self.event_tensor_full = None  # Full resolution frames
        self.output_folder = None  # Will be auto-set when file is selected
    ...
    def generate_frames(self):
        """Generate and save accumulated frames"""
        ...
        def frame_worker():
            try:
                self.update_status("Generating frames...")
                self.log_result("\n" + "="*70)
                self.log_result("FRAME GENERATION & SAVING")
                self.log_result("="*70)
                
                x, y, t, p = self.events_data
                
                # Apply compensation and create accumulated frames using index_put
                xs = torch.tensor(x, device=device)
                ys = torch.tensor(y, device=device)
                ts = torch.tensor(t, device=device)
                ps = torch.tensor(p, device=device)
                ...
                # Use the frame-only generation method (no gradients, can use index_put)
                event_tensor = self.model.generate_frames_only(xs, ys, ts, ps, 720, 1280, self.accumulate_var.get())
                self.event_tensor_full = event_tensor.detach().cpu()
                
                num_frames, height, width = event_tensor.shape
                ...
                frame_folder = os.path.join(self.output_folder, f"{base_filename}_frames")
                os.makedirs(frame_folder, exist_ok=True)
                
                # Save each frame
                self.log_result(f"💾 Saving frames to: {frame_folder}")
                
                # Create wavelength mapping
                wavelength_range = self.max_wave_var.get() - self.min_wave_var.get()
                
                for i in range(num_frames):
                    # Extract frame as tensor (avoid numpy operations)
                    frame_tensor = event_tensor[i]
                    
                    # Get max value using PyTorch
                    frame_max = frame_tensor.max().item()
                    
                    # Convert to numpy only for PIL Image creation
                    if frame_max > 0:
                        # Normalize using PyTorch operations
                        frame_norm_tensor = (frame_tensor / frame_max * 255)
                        frame_norm = frame_norm_tensor.byte().cpu().numpy()
                    else:
                        frame_norm = torch.zeros_like(frame_tensor, dtype=torch.uint8).cpu().numpy()
                    
                    # Calculate wavelength
                    wavelength = self.min_wave_var.get() + (i / max(1, num_frames-1)) * wavelength_range
                    
                    # Save as PNG
                    filename = f"frame_{i:03d}_wavelength_{wavelength:.0f}nm.png"
                ...
                self.log_result(f"✅ All {num_frames} frames saved successfully!")
                self.log_result(f"📁 Output folder: {frame_folder}")
                ...
                self.update_status("✅ Frames generated & saved")
                self.update_progress(100)
    ...
    def show_3d_stack(self):
        """Show 3D stack visualization with proper time stretching"""
        if self.event_tensor_full is None:
            messagebox.showerror("Error", "Please generate frames first")
            return
        ...
        try:
            ...
            x_coords = list(range(0, width, downsample))
            y_coords = list(range(0, height, downsample))
    ...
    def show_frame_grid(self):
        ...
        try:
            ...
            grid_window.title("All Spectral Frames")
            ...
            self.log_result(f"✅ Frame grid created: {num_frames} frames in {rows}x{cols} grid")
