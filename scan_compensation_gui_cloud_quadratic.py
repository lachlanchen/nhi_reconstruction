...
Enhanced with simplified quadratic time-dependent compensation: phi(t) = a(t-b)^2+1
where b is automatically set to half the time range, and only 'a' is learned
"""
...
class ScanCompensation(nn.Module):
    def __init__(self, initial_params):
        super().__init__()
        # Now using 4 parameters: [a_x, a_y, a_quad, b_quad]
        # All parameters are learnable
        self.params = nn.Parameter(initial_params)
        
    def warp(self, x_coords, y_coords, timestamps):
        """Adjust timestamps based on x, y positions with quadratic time function"""
        a_x = self.params[0]
        a_y = self.params[1]
        # Constrain a_quad to reasonable positive values to prevent explosion
        a_quad = torch.sigmoid(self.params[2]) * 1e-6  # Larger but still controlled positive value
        # Constrain b_quad to positive values
        b_quad = torch.abs(self.params[3])  # Ensure b_quad > 0
        
        # Normalize time by subtracting start time
        t_start = timestamps.min()
        t_normalized = timestamps - t_start
        
        # Calculate phi(t) = a*(t-b)^2 + 1
        # Now both a_quad and b_quad are learnable
        phi_t = a_quad * (t_normalized - b_quad)**2 + 1
        
        # Apply time-dependent compensation: t' = t - (ax*x + ay*y) * phi(t)
        t_warped = timestamps - (a_x * x_coords + a_y * y_coords) * phi_t
...
class ScanCompensationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Scan Compensation with Quadratic Function")
    ...
    def load_config(self):
        ...
        self.config = {
            ...
            "ax_value": 0.0,
            "ay_value": 0.0,
            "a_quad_value": 0.0,  # Will be passed through sigmoid, so 0 = small positive
            "b_quad_value": 500000.0,  # Default to 500ms, will be auto-set to half time range
    ...
    def save_config(self):
        ...
        try:
            self.config.update({
                ...
                "a_quad_value": self.a_quad_var.get(),
                "b_quad_value": self.b_quad_var.get(),
    ...
    def create_widgets(self):
        ...
        # Quadratic Parameters - Row 2, Column 2 (expanded)
        quad_frame = ttk.LabelFrame(control_frame, text="Quadratic φ(t) = a(t-b)²+1", padding=5)
        quad_frame.grid(row=1, column=1, sticky="ew", padx=2, pady=2)
        
        ttk.Label(quad_frame, text="a_quad:").grid(row=0, column=0, sticky="w")
        self.a_quad_var = tk.DoubleVar(value=self.config["a_quad_value"])
        ttk.Entry(quad_frame, textvariable=self.a_quad_var, width=12).grid(row=0, column=1, padx=2)
        
        ttk.Label(quad_frame, text="b_quad (μs):").grid(row=1, column=0, sticky="w")
        self.b_quad_var = tk.DoubleVar(value=self.config["b_quad_value"])
        ttk.Entry(quad_frame, textvariable=self.b_quad_var, width=12).grid(row=1, column=1, padx=2)
        ...
        self.enable_training_var = tk.BooleanVar(value=self.config["enable_training"])
        ...
        self.iterations_var = tk.IntVar(value=500)  # Reduced default
        ...
        self.opt_bin_var = tk.DoubleVar(value=200000)  # Larger bins to reduce memory
        ...
        # Auto-save buttons - Row 3, Column 4-6
        ...
        ttk.Button(save_frame, text="Plot φ(t)", command=self.plot_phi_function).grid(row=0, column=2, sticky="ew", padx=2)
        ttk.Button(save_frame, text="Auto Set Params", command=self.auto_set_params).grid(row=1, column=0, columnspan=3, sticky="ew", padx=2)
        ...
        self.status_var = tk.StringVar(value="Ready - Quadratic function with learnable a_quad and b_quad")
        ...
        # Log initialization
        self.log_result("Enhanced Scan Compensation with Quadratic Function")
        self.log_result(f"Device: {device}")
        self.log_result(f"Quadratic function: φ(t) = a_quad*(t-b_quad)² + 1")
        self.log_result(f"Both a_quad and b_quad are learnable parameters")
        self.log_result(f"b_quad defaults to half of time range")
        self.log_result(f"a_quad parameter scaling: 1e-6 for meaningful effect")
        self.log_result(f"Configuration loaded from: {self.config_file}")
        
    def plot_phi_function(self):
        """Plot the quadratic phi(t) function to visualize time-dependent scaling"""
        if self.events_data is None:
            messagebox.showwarning("Warning", "Please load data first to see time range")
            return
            
        try:
            x, y, t, p = self.events_data
            
            # Create a new window for phi(t) plot
            phi_window = tk.Toplevel(self.root)
            phi_window.title("Quadratic Time Function φ(t)")
            phi_window.geometry("800x600")
            
            fig = Figure(figsize=(10, 6))
            ax = fig.add_subplot(111)
            
            # Get parameters - use actual values from model if available
            if self.compensation_params and len(self.compensation_params) >= 4:
                a_quad = self.compensation_params[2]
                b_quad = self.compensation_params[3]
            else:
                # Use current UI values
                a_quad = torch.sigmoid(torch.tensor(self.a_quad_var.get())) * 1e-6
                a_quad = float(a_quad)
                b_quad = self.time_range / 2.0 if self.time_range else (t.max() - t.min()) / 2.0
            
            # Time range from data
            t_start = t.min()
            t_end = t.max()
            t_normalized = np.linspace(0, t_end - t_start, 1000)
            
            # Calculate phi(t)
            phi_t = a_quad * (t_normalized - b_quad)**2 + 1
            
            # Plot
            ax.plot(t_normalized / 1e6, phi_t, 'b-', linewidth=2, label=f'φ(t) = {a_quad:.2e}*(t-{b_quad:.0e})² + 1')
            ax.axhline(y=1, color='r', linestyle='--', alpha=0.7, label='φ(t) = 1 (no scaling)')
            ax.set_xlabel('Time - Start Time (seconds)')
            ax.set_ylabel('φ(t) - Time Scaling Factor')
            ax.set_title('Quadratic Time-Dependent Scaling Function')
            ...
            # Add canvas
            canvas = FigureCanvasTkAgg(fig, phi_window)
            canvas.draw()
            canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            
            # Add toolbar
            toolbar = NavigationToolbar2Tk(canvas, phi_window)
            toolbar.update()
            
            self.log_result(f"φ(t) plotted: a_quad={a_quad:.2e}, b_quad={b_quad:.0e}")
            self.log_result(f"Scaling range: {phi_min:.3f} to {phi_max:.3f} (mean: {phi_mean:.3f})")
            
        except Exception as e:
            self.log_result(f"Error plotting φ(t): {str(e)}")
    ...
    def auto_set_params(self):
        ...
        if self.file_path_var.get():
            self.ax_var.set(0.0)
            self.ay_var.set(0.0)
            self.a_quad_var.set(0.0)  # Will give small positive value through sigmoid
            # Auto-set b_quad to half time range if we have data loaded
            if self.time_range:
                self.b_quad_var.set(self.time_range / 2.0)
                self.log_result(f"Parameters auto-set: b_quad={self.time_range/2.0:.0f} μs (half time range)")
            else:
                self.b_quad_var.set(500000.0)  # Default 500ms
                self.log_result("Parameters auto-set: b_quad=500000 μs (default)")
            self.log_result("Other params: ax=0.0, ay=0.0, a_quad=0.0")
    ...
    def auto_load_worker(self):
        ...
        try:
            ...
            self.time_range = float(t.max() - t.min())
            
            # Auto-set b_quad to half time range if not set
            if self.b_quad_var.get() == self.config["b_quad_value"]:  # If still default
                self.b_quad_var.set(self.time_range / 2.0)
            
            # Auto-set parameters (now 4 parameters - all learnable)
            manual_params = torch.tensor([
                self.ax_var.get(), 
                self.ay_var.get(),
                self.a_quad_var.get(),  # Will be passed through sigmoid in model
                self.b_quad_var.get()   # Now learnable
            ], device=device, requires_grad=False)
            self.model = ScanCompensation(manual_params)
            
            # Get actual a_quad value from model
            actual_a_quad = torch.sigmoid(torch.tensor(self.a_quad_var.get())) * 1e-6
            self.compensation_params = [
                self.ax_var.get(), 
                self.ay_var.get(),
                float(actual_a_quad),
                self.b_quad_var.get()  # Now learnable b_quad
            ]
            
            self.log_result(f"Auto-loaded: {len(x):,} events")
            self.log_result(f"Time range: {self.time_range/1e6:.3f} seconds")
            self.log_result(f"Parameters set: ax={self.ax_var.get():.6f}, ay={self.ay_var.get():.6f}")
            self.log_result(f"a_quad={float(actual_a_quad):.2e}, b_quad={self.b_quad_var.get():.0f} μs")
        ...
        def optimization_worker():
            try:
                ...
                if self.enable_training_var.get():
                    self.log_result("QUADRATIC SCAN COMPENSATION OPTIMIZATION")
                else:
                    self.log_result("MANUAL QUADRATIC PARAMETER SETTING")
                self.log_result("="*60)
                ...
                self.log_result(f"Duration: {self.time_range/1e6:.3f} seconds")
                # Auto-set b_quad to half time range if not manually set
                if self.b_quad_var.get() == self.config["b_quad_value"]:  # If still default
                    self.b_quad_var.set(self.time_range / 2.0)
                    self.log_result(f"b_quad auto-set to: {self.time_range/2.0:.0f} μs (half of range)")
                else:
                    self.log_result(f"b_quad (manual): {self.b_quad_var.get():.0f} μs")
                ...
                if self.enable_training_var.get():
                    # Run optimization with 4 parameters (all learnable)
                    self.update_status("Running quadratic optimization...")
                    
                    # Auto-set b_quad to half time range if not manually set
                    if self.b_quad_var.get() == self.config["b_quad_value"]:  # If still default
                        self.b_quad_var.set(self.time_range / 2.0)
                    
                    # Initialize model with manual values as starting point (4 parameters now)
                    initial_params = torch.tensor([
                        self.ax_var.get(), 
                        self.ay_var.get(),
                        self.a_quad_var.get(),  # Will be passed through sigmoid
                        self.b_quad_var.get()   # Now learnable
                    ...
                    # Get initial actual values
                    with torch.no_grad():
                        initial_a_quad = torch.sigmoid(self.model.params[2]) * 1e-6
                        initial_b_quad = torch.abs(self.model.params[3])
                    
                    self.log_result(f"\nOptimization parameters:")
                    self.log_result(f"  Initial a_x: {self.ax_var.get():.6f} μs/pixel")
                    self.log_result(f"  Initial a_y: {self.ay_var.get():.6f} μs/pixel")
                    self.log_result(f"  Initial a_quad: {float(initial_a_quad):.2e}")
                    self.log_result(f"  Initial b_quad: {float(initial_b_quad):.0f} μs")
                    ...
                    for i in range(num_iterations):
                        ...
                        if i % 50 == 0:  # More frequent updates
                            ...
                            actual_a_quad = torch.sigmoid(params[2]) * 1e-6
                            actual_b_quad = torch.abs(params[3])
                            self.log_result(f"  {i:4d}: Loss={loss.item():.6f}, "
                                          f"a_x={params[0].item():.6f}, a_y={params[1].item():.6f}, "
                                          f"a_quad={float(actual_a_quad):.2e}, b_quad={float(actual_b_quad):.0f}")
                    ...
                    final_a_quad = torch.sigmoid(final_params[2]) * 1e-6
                    final_b_quad = torch.abs(final_params[3])
                    self.compensation_params = [
                        final_params[0].item(), 
                        final_params[1].item(),
                        float(final_a_quad),
                        float(final_b_quad)  # Now learnable b_quad
                    ]
                    
                    self.log_result(f"\nQUADRATIC OPTIMIZATION COMPLETE!")
                    ...
                    self.log_result(f"  a_quad = {self.compensation_params[2]:.2e}")
                    self.log_result(f"  b_quad = {self.compensation_params[3]:.0f} μs")
                    ...
                    self.b_quad_var.set(self.compensation_params[3])
                    # Don't update a_quad_var as it's the raw value before sigmoid
                    
                else:
                    # Use manual parameters directly (3 parameters now)
                    self.update_status("Setting manual simplified quadratic parameters...")
                    ...
                    self.model = ScanCompensation(manual_params, self.time_range)
                    
                    # Get actual a_quad value
                    actual_a_quad = torch.sigmoid(torch.tensor(self.a_quad_var.get())) * 1e-10
                    self.compensation_params = [
                        self.ax_var.get(), 
                        self.ay_var.get(),
                        float(actual_a_quad),
                    ...
                    self.log_result(f"\nMANUAL SIMPLIFIED QUADRATIC PARAMETERS SET:")
                    self.log_result(f"  a_x = {self.compensation_params[0]:.6f} μs/pixel")
                    self.log_result(f"  a_y = {self.compensation_params[1]:.6f} μs/pixel")
                    self.log_result(f"  a_quad = {self.compensation_params[2]:.2e}")
                    self.log_result(f"  b_quad = {self.compensation_params[3]:.0f} μs (auto-set)")
                    self.log_result(f"  No optimization performed")
                
                # Auto-save configuration
                self.save_config()
                
                self.update_status("Quadratic parameters ready for 3D visualization")
    ...
    def generate_3d_vis(self):
        ...
        try:
            ...
            self.log_result("\n" + "="*60)
            self.log_result("3D SPECTRAL VISUALIZATION WITH QUADRATIC FUNCTION")
            self.log_result("="*60)
            ...
            self.log_result(f"Using quadratic parameters:")
            self.log_result(f"  a_x = {self.compensation_params[0]:.6f} μs/pixel")
            self.log_result(f"  a_y = {self.compensation_params[1]:.6f} μs/pixel")
            self.log_result(f"  a_quad = {self.compensation_params[2]:.2e}")
            self.log_result(f"  b_quad = {self.compensation_params[3]:.0f} μs")
            self.log_result(f"  φ(t) = {self.compensation_params[2]:.2e}*(t-{self.compensation_params[3]:.0f})² + 1")
        ...
        try:
            ...
            if len(ts_sampled) > 0:
                ...
                if is_projection:
                    ...
                    ax.set_title(f'3D Compensated Events - {mode} Quadratic ({view_name})\n'
                               f'ax={self.compensation_params[0]:.3f}, ay={self.compensation_params[1]:.3f}, '
                               f'a_quad={self.compensation_params[2]:.1e}, b_quad={self.compensation_params[3]:.0e}\n'
