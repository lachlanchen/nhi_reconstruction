...
class ScanCompensation(nn.Module):
    ...
    def generate_compensated_frames(self, x_coords, y_coords, timestamps, polarities, H, W, bin_width, manual_params=None):
        """Generate frames with manual compensation parameters or no compensation"""
        with torch.no_grad():
            if manual_params is not None:
                # Apply specified compensation (could be [0,0] for original or actual compensation values)
                x_warped, y_warped, t_warped = self.warp(x_coords, y_coords, timestamps, manual_params)
            else:
                # Use learned compensation parameters
                x_warped, y_warped, t_warped = self.warp(x_coords, y_coords, timestamps)
            ...
            print(f"DEBUG: Time range {t_start.item():.0f} - {t_end.item():.0f} μs, bin_width={bin_width}, calculated bins={num_bins}")
            ...
            # Debug: Print actual tensor shape
            print(f"DEBUG: Created tensor shape: {event_tensor.shape}, expected: ({num_bins}, {H}, {W})")
...
class ScanCompensationGUI:
    ...
    def apply_manual_params(self):
        ...
        def manual_worker():
            try:
                ...
                # Calculate variance improvement - use actual tensor shapes with safety checks
                try:
                    actual_frames_orig = self.event_tensor_original.shape[0]
                    actual_frames_comp = self.event_tensor_compensated.shape[0]
                    
                    self.log_result(f"📊 Tensor shapes: Original={self.event_tensor_original.shape}, Compensated={self.event_tensor_compensated.shape}")
                    
                    if actual_frames_orig > 0 and actual_frames_comp > 0:
                        var_orig = torch.var(self.event_tensor_original.view(actual_frames_orig, -1), dim=1).mean()
                        var_comp = torch.var(self.event_tensor_compensated.view(actual_frames_comp, -1), dim=1).mean()
                        improvement = ((var_orig - var_comp) / var_orig * 100).item()
                        
                        self.log_result(f"📈 Variance improvement: {improvement:.1f}%")
                    else:
                        improvement = 0.0
                        self.log_result(f"⚠️ Cannot calculate variance - empty tensors")
                        
                except Exception as var_error:
                    improvement = 0.0
                    self.log_result(f"⚠️ Variance calculation error: {str(var_error)}")
    ...
    def show_3d_stack(self):
        ...
        try:
            ...
            # Get compensated data with safety checks
            tensor = self.event_tensor_compensated
            if tensor is None or tensor.numel() == 0:
                self.log_result("❌ No compensated tensor data available")
                return
                
            num_frames, height, width = tensor.shape
            
            if num_frames == 0:
                self.log_result("❌ No frames in compensated tensor")
                return
    ...
    def show_frame_grid(self):
        ...
        try:
            ...
            # Create figure with safety checks
            tensor = self.event_tensor_compensated
            if tensor is None or tensor.numel() == 0:
                self.log_result("❌ No compensated tensor data available")
                messagebox.showerror("Error", "No compensated tensor data available")
                return
    ...
    def show_comparison(self):
        ...
        try:
            ...
            # Select a middle frame for comparison with safety checks
            num_frames_orig = self.event_tensor_original.shape[0]
            num_frames_comp = self.event_tensor_compensated.shape[0]
            num_frames = min(num_frames_orig, num_frames_comp)
            
            if num_frames == 0:
                messagebox.showerror("Error", "No frames available for comparison")
                return
            
            frame_idx = num_frames // 2
            self.log_result(f"📊 Comparison: Using frame {frame_idx} of {num_frames} available frames")
    ...
    def show_variance_plot(self):
        ...
        try:
            ...
            # Calculate variances - use actual tensor shapes
            num_frames_orig = self.event_tensor_original.shape[0]
            num_frames_comp = self.event_tensor_compensated.shape[0]
            num_frames = min(num_frames_orig, num_frames_comp)
            
            self.log_result(f"📊 Variance plot: Using {num_frames} frames (orig={num_frames_orig}, comp={num_frames_comp})")
            
            var_orig_list = []
            var_comp_list = []
            
            for i in range(num_frames):
                if i < num_frames_orig and i < num_frames_comp:
