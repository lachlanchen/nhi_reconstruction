...
def save_event_frames(model, x, y, t, p, H, W, bin_width, original_t_start, original_t_end, output_dir, filename_prefix):
    """
    Save both original and compensated event frames as numpy arrays and images
    """
    print("Generating and saving event frames...")
    
    # Create frames directory
    frames_dir = os.path.join(output_dir, f"{filename_prefix}_frames")
...
def create_frame_comparison_plots(frames_orig, frames_comp, output_dir, filename_prefix, 
                                 max_frames_to_show=None, skip_frames=None):
    """
    ...
    comparison_dir = os.path.join(output_dir, f"{filename_prefix}_frame_comparisons")
...
def main():
    ...
    parser.add_argument('--save_frames', action='store_true', help='Save all event frames as images and arrays')
    ...
    # Save frames and create comparisons if requested
    if args.save_frames:
        ...
        create_frame_comparison_plots(
            ...
            max_frames_to_show=args.max_comparison_frames
        )
